...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_seq_len', default=256, type=int, help="训练的最大截断长度（中文1token≈1.5~1.7字符）")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument("--data_path", type=str, default="../dataset/pretrain_hq.jsonl", help="预训练数据路径")
    parser.add_argument("--max_samples", type=int, default=50000, help="最多使用多少条训练样本，0表示全部使用")
    ...
    if args.max_samples and args.max_samples > 0:
        max_n = min(args.max_samples, len(train_ds))
        train_ds = Subset(train_ds, list(range(max_n)))
        Logger(f'只使用前 {max_n} 条样本进行训练')
